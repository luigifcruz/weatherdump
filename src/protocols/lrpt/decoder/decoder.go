...
func NewDecoder(uuid string) interfaces.Decoder {
	...
	if uuid != "" {
		http.HandleFunc(fmt.Sprintf("/socket/lrpt/%s", uuid), e.socketsHandler)
		e.uuid = uuid
	}
...
func (e *Worker) updateSockets(s assets.Statistics) {
...
func (e *Worker) socketsHandler(w http.ResponseWriter, r *http.Request) {
